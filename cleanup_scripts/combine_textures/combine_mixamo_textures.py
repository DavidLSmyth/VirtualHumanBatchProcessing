...
def gen_obj_single_texture(fbx_file_path, obj_file_path):
    #1. read in fbx file
...
def gen_single_obj_texture_folder(fbx_folder, obj_folder):
    for fbx_file in os.listdir(fbx_folder):
        print("Processing ", fbx_file)
        gen_obj_single_texture(os.path.join(fbx_folder, fbx_file), os.path.join(obj_folder, os.path.splitext(fbx_file)[0] + ".obj"))
        set_empty_scene()

def main():
    #1. read in fbx file

    fbx_folder_loc = r"D:\SAUCEFiles\Microsoft-Rocketbox-master\Microsoft-Rocketbox-master\Assets\Avatars\Professions"
    ...
    for file in files_to_process:
        assert os.path.exists(os.path.join(fbx_folder_loc, file)), file


    for mesh_name in files_to_process:

        gen_obj_single_texture(os.path.join(fbx_folder_loc, "{}".format(mesh_name)),
                               os.path.join(obj_folder_loc, "{}.obj".format(mesh_name[:-4])))

        #create a new directory and move there
        newdir = os.path.join(obj_folder_loc, mesh_name[:-4])
        os.mkdir(newdir)
