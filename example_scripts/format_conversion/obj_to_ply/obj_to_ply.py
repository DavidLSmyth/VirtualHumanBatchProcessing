...
def get_arg_parser():
    ...
    parser.add_argument('-ply_file', type = str,
                        help='The folder containing the converted ply file.')
    return parser


def obj_to_ply_example(obj_file_path, ply_file_path, use_anmiation = False):
    '''
    Use a function like this to set the import/export params
    :param obj_file_path: path to the obj file to import
    :param ply_file_path: path to the ply file to export
    :return:
    '''
    import_args_manager = ImportArgsManager(import_file_type='obj')
    export_args_manager = ExportArgsManager(export_file_type="ply")
    import_args_manager.set_import_arg("filepath", obj_file_path)

    export_args_manager.set_export_arg("filepath", ply_file_path)
...
def main():
    ...
    reset_blend()
    convert_args_manager(*obj_to_ply_example(args.obj_file, args.ply_file, True))
